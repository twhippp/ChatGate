...
from PyQt5.QtCore import QThread, pyqtSignal, Qt, QTimer, QPoint, QUrl
from PyQt5.QtGui import QDesktopServices
from overlay import ChatOverlay

# ===================== CONFIG =====================
SETTINGS_FILE = "settings.json"
CURRENT_VERSION = "0.2.1-beta" # Set lower than your GitHub tag to test green text
                ...
                for line in data.split('\r\n'):
                    ...
                    if "PRIVMSG" in line:
                        ...
                        if line.startswith("@"):
                            ...
                            if len(parts) > 1:
                                tags = dict(t.split("=") for t in parts[0].split(";") if "=" in t)

                        user = tags.get("display-name", "User")
                        content_parts = line.split('PRIVMSG', 1)
                        content = content_parts[1].split(':', 1)[1] if len(content_parts) > 1 else ""
...
class ChatGateMain(QWidget):
    ...
    def load_settings(self):
        defaults = {"channel": "piratesoftware", "mps": 3.0, "opacity": 80, "mod": True, "vip": True, "sub": False, "pos_x": 100, "pos_y": 100, "width": 400, "height": 600}
    ...
    def save_settings(self):
        data = {"channel": self.chan_input.text(), "mps": self.mps_spin.value(), "opacity": self.alpha_slider.value(), "mod": self.bp_mod.isChecked(), "vip": self.bp_vip.isChecked(), "sub": self.bp_sub.isChecked(), "pos_x": self.overlay.x(), "pos_y": self.overlay.y(), "width": self.overlay.width(), "height": self.overlay.height()}
        with open(SETTINGS_FILE, "w") as f: json.dump(data, f, indent=4)

    def apply_theme(self):
        self.setStyleSheet("QWidget { background-color: #18181b; color: #efeff1; font-family: 'Segoe UI'; } QLineEdit { background-color: #26262c; border: 1px solid #464649; color: white; padding: 5px; } QPushButton { background-color: #9146FF; color: white; font-weight: bold; padding: 10px; }")
    ...
    def init_ui(self):
        ...
        stats.addWidget(self.mps_label); stats.addStretch(); stats.addWidget(QLabel("Threshold:")); stats.addWidget(self.mps_spin)
        ...
        layout.addWidget(QLabel("<b>WIDTH</b>"))
        self.width_slider = QSlider(Qt.Horizontal)
        self.width_slider.setRange(200, 1200); self.width_slider.setValue(self.settings.get("width", 400))
        self.width_slider.valueChanged.connect(self.sync_overlay)
        layout.addWidget(self.width_slider)

    def sync_overlay(self):
        self.overlay.update_style(22, self.alpha_slider.value() / 100.0)
    ...
    def toggle_move_mode(self):
        ...
        self.move_btn.setText("LOCK POSITION" if is_unlocked else "UNLOCK OVERLAY TO MOVE")
        if is_unlocked: self.overlay.show()
        else: self.save_settings()

    def check_for_updates(self):
        print("DEBUG: --- Beta Tag Check Started ---")
        try:
            url = f"https://api.github.com/repos/{GITHUB_REPO}/tags"
            req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 (ChatGate-Updater)'})
            ...
            with urllib.request.urlopen(req, timeout=5) as r:
                ...
                if data and len(data) > 0:
                    ...
                    print(f"DEBUG: Remote Tag: {latest_tag} | Local Version: {CURRENT_VERSION}")

                    if version.parse(latest_v) > version.parse(current_v):
                        print("DEBUG: Higher beta version found!")
                        # Linking to tags page since no formal releases exist yet
                        update_link = f"https://github.com/{GITHUB_REPO}/tags"
                        self.ver_label.setText(f"<a href='{update_link}' style='color:#00ff7f; text-decoration:none;'>UPDATE AVAILABLE: {latest_tag}</a>")
                        self.ver_label.setOpenExternalLinks(True)
                    else:
                        print("DEBUG: No newer version available.")
                        self.ver_label.setText(f"v{CURRENT_VERSION} (Latest)")
                else:
                    print("DEBUG: No tags found on repository.")
        except Exception as e:
            print(f"DEBUG: Update Check FAILED: {e}")
        print("DEBUG: --- Beta Tag Check Finished ---")

    def register_hotkey(self):
        try:
            ctypes.windll.user32.RegisterHotKey(int(self.winId()), HOTKEY_ID, 0x0002 | 0x0004, ord("O"))
        except:
            print("DEBUG: Hotkey registration skipped (Non-Windows Environment)")
